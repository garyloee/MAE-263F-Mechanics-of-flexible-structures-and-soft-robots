...
xlabel('x [meter]');
ylabel('y [meter]');
title('Shape of Structure at Specified Time')
...
for n=1:steps
    ...
    if ismember((n-1)*dt,plotPoints)
        hold on
        leg=sprintf('Time = %2.2f s',(n-1)*dt);
...
hold on
plot(10, all_mid_v(n), 'r.', 'MarkerFaceColor','r')
hold off
text(10, all_mid_v(n), sprintf('(Terminal Velocity = %f m/s)',all_mid_v(n)), 'Horiz','right', 'Vert','bottom')
...
xlabel('Time, t [sec]');
ylabel('Turning Angle, θ [radian]');
title('Mid-node Turning Angle')
hold on
plot(10, angles(n), 'b.', 'MarkerFaceColor','r')
hold off
text(10, angle(n), sprintf('(Terminal Turning Angle = %f rad)',angles(n)), 'Horiz','right', 'Vert','bottom')
toc
