function problem2Temp()
clear variables
% Declare the amount of Spheres (N)

Tstep=[0.5,0.1,0.05,0.01,0.005,0.001];
TM=zeros(length(Tstep),1); % creating space for terminal velocity
for p=1:length(Tstep)
    N=21;
    ndof=N*2; % number of degree of freedom
    
    dt=Tstep(p); % Time step (second)
    ...
    for n=1:steps
        %fprintf('Time = %f\n',(n-1)*dt); 
        % stop printing ttime to save time
    ...
    %}
    m=(N+1)/2;
    fprintf('Terminal Velocity of dt = %.3f : %f meters/sec\n',Tstep(p), u(2*m))
    %saving terminal velocity
    TM(p)=u(2*m);
end
...
figure(7)
plot(Tstep,TM,'.-');
xlabel('Time step size, dt [second]');
ylabel('Terminal Velocity of Mid-node, v [meter/sec]');
ytickformat('%.2f')
ylim([-5.9300e-3 -5.8300e-3])
title('Terminal Velocity at Varying Time Discretization')
